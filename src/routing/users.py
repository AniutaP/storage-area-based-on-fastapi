...
from src.schemas.users import UserAddSchema, UserSchema, UserUpdateSchema, UserWithOrdersSchema, UserIdSchema
...
async def get_by_id_with_orders(
    ...
    user = await user_service.get_by_id_with_orders(id=user_id, order_service=order_service, db_session=db_session)
...
async def update_by_id(
    ...
    if current_user.id == user_dto.id or current_user.is_superuser:
        user_to_update = await user_service.update_by_id(user=user_dto, db_session=db_session)
